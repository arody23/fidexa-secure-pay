...
VALUES (
  ...
  true,
  5242880, -- 5MB limit
  ARRAY['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'audio/mpeg', 'audio/ogg', 'audio/wav']::text[]
)
...
-- Clients anonymes (page commande sans compte)
DROP POLICY IF EXISTS "Anon can upload support attachments" ON storage.objects;
CREATE POLICY "Anon can upload support attachments"
ON storage.objects FOR INSERT
TO anon
WITH CHECK (bucket_id = 'support-attachments');
...
-- Vérification
SELECT 
    'Bucket support-attachments créé ✅' as status,
    id,
    name,
    public,
...
-- ================================================================
-- FIX RLS POUR LA TABLE USERS - MISE À JOUR DU PROFIL
-- Résout: "Échec de la mise à jour. Vérifiez vos permissions."
-- ================================================================

-- 1. Nettoyer les anciennes policies conflictuelles
DROP POLICY IF EXISTS "users_update_own" ON public.users;
DROP POLICY IF EXISTS "Users can update own profile" ON public.users;
DROP POLICY IF EXISTS "users_update_own_or_admin" ON public.users;
DROP POLICY IF EXISTS "users_update_policy" ON public.users;

-- 2. Créer une policy UPDATE claire et simple
...
-- 3. S'assurer que RLS est bien activé
ALTER TABLE public.users ENABLE ROW LEVEL SECURITY;

-- 4. Vérifier les policies existantes pour SELECT (lecture)
-- Si aucune policy SELECT n'existe, en créer une
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 5. Vérification finale - Afficher toutes les policies sur users
SELECT 
    schemaname,
    tablename,
    policyname,
...
SELECT 
    'RLS configuré pour la table users ✅' as status,
...
-- ================================================================
-- SOLUTION SIMPLE: Désactiver RLS sur Storage (temporaire)
-- Pour débloquer l'upload d'avatars immédiatement
-- ================================================================

-- IMPORTANT: Cette solution fonctionne mais est moins sécurisée
-- À utiliser temporairement si les policies ne peuvent pas être créées

-- 1. Vérifier que le bucket avatars existe et est PUBLIC
SELECT 
    id,
    name,
    public as est_public,
    CASE 
        WHEN public = true THEN '✅ Le bucket est PUBLIC - PARFAIT!'
        ELSE '❌ Le bucket doit être PUBLIC - Modifier dans Dashboard'
    END as statut
FROM storage.buckets 
WHERE name = 'avatars';

-- 2. DÉSACTIVER RLS sur storage.objects (permet tout)
...
SELECT 
    ...
    tablename,
    CASE 
        WHEN rowsecurity = false THEN '✅ RLS DÉSACTIVÉ - L''upload devrait marcher maintenant!'
        ELSE '⚠️ RLS encore activé'
    END as statut_rls
FROM pg_tables 
...
-- 4. Message final
SELECT 
    '🎉 CONFIGURATION SIMPLIFIÉE TERMINÉE!' as titre,
    'RLS désactivé sur Storage - Testez l''upload d''avatar maintenant' as instruction,
    'Cette config fonctionne mais est moins sécurisée (tout le monde peut uploader)' as avertissement
;

-- ================================================================
-- POUR RÉACTIVER LA SÉCURITÉ PLUS TARD (quand les policies marchent):
-- ================================================================
-- 
-- ALTER TABLE storage.objects ENABLE ROW LEVEL SECURITY;
-- 
-- Puis créer les policies via Dashboard Storage > Policies
-- ================================================================
